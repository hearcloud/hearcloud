    ...
    jQuery.extend({
        fm: function (custom_options) {
            ...
            var defaults = {
                ...
                update_body_selector: ".update-form-body",
            ...
            var update_body = $(global_options.update_body_selector);
            ...
            function init() {
                ...
                delegate_target.on('click', global_options.update_button_selector, function () {
                    ...
                    var options = extract_modal_options(self, 'update');
                    debug("options:");
                    debug(options);
                    modal_buttons.find('[btn-edit-song]').unbind('click').bind("click", function () {
                        var form = modal.find('form');
                        submit_form(form, options);
                    });
                    ...
                    $.ajax({
                        ...
                        error: function () {
                            debug("error occurred while loading modal body from URL");
                            update_body.text(global_options.modal_load_error);
                        },
                        success: function (data) {
                            debug("modal body successfully loaded");
                            update_body.html(data);
                            var form = update_body.find('form');
                            //form.removeClass().addClass('form-horizontal');
                            form.attr('id', 'song-details-form');
                            //form.find('label').removeClass().addClass('control-label col-sm-2');
                            //form.find('input').removeClass().addClass('col-sm-4');
                            form.on('submit', function () {
                                submit_form(form, options);
                                return false;
                            });
